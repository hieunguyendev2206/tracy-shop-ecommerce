...
namespace TracyShop.Controllers
{
    public class ProductController : Controller
    {
        ...
        public ActionResult Details(int id)
        {
            ...
            foreach (var pr in qr3) sizes.Add(qr1.Where(d => d.Id == pr.SizeId).First());
            ...
            // Lấy khuyến mãi
            var promotions = _context.Promotion.Where(p => p.StartedDate <= DateTime.Now && p.EndDate >= DateTime.Now)
                .ToList();
        ...
        public async Task<ActionResult> AddCart(int id)
        {
            ...
            var totalQuantity = 0;
            foreach (var item in quantities) totalQuantity += item.Quantity;

            if (totalQuantity > 0)
            {
                // Lấy khuyến mãi
                var promotions = _context.Promotion
                ...
                var carts = _context.Carts
                    .Where(c => c.UserId == userid && c.IsBuy == false && c.SelectedSize == qr3.SizeId).ToList();
        ...
        public async Task<ActionResult> BuyNow(int id)
        {
            ...
            if (qr3.Count > 0)
            {
                ...
                // Kiểm tra sp đã có trong giỏ hàng chưa, nếu có thì tăng số lượng sp trong giỏ, nếu chưa thì thêm sp vào giỏ
                var carts = _context.Carts
                    .Where(c => c.UserId == userid && c.IsBuy == false && c.SelectedSize == size.Id).ToList();
                if (carts.Where(c => c.ProductId == id).ToList().Count == 0)
                {
                    var cart = new Cart();
                    if (ModelState.IsValid)
