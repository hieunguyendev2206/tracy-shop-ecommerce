...
namespace TracyShop.Controllers
{
    public class OrderController : Controller
    {
        ...
        public OrderController(ILogger<OrderController> logger, AppDbContext context,
            ...
            ILoginRepository loginRepository, IConfiguration config)
        {
            _logger = logger;
            _context = context;
        ...
        public IActionResult Order()
        {
            ...
            if (address != null)
                orderView.Address = string.Format("{0}, {1}, {2}", address.SpecificAddress, address.District,
                    address.City);
            else
                orderView.Address = null;
            orderView.PhoneNumber = user.PhoneNumber;
            if (user.PhoneNumber == null) ViewBag.Message = "Thêm số điện thoại";
        ...
        public async Task<IActionResult> Order(OrderViewModel orderView)
        {
            if (orderView.PaymentMenthodId == 1)
                try
                {
                    ...
                    foreach (var item in cart)
                    {
                        ...
                        var query = _context.ProductSize
                            .Where(p => p.ProductId == item.ProductId && p.SizeId == item.SelectedSize).First();
...

            {
                ...
                var cart = _context.Carts
                    .Where(c => c.UserId == _userManager.GetUserId(HttpContext.User) && c.IsBuy == false).ToList();
                float totalPrice = 0;
                foreach (var item in cart) totalPrice += item.Quantity * item.UnitPrice * (1 - item.Promotion);
