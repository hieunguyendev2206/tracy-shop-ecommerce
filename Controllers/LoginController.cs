    ...
    public class LoginController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterModel userModel)
        {
            if (ModelState.IsValid)
            {
                ...
                if (!result.Succeeded)
                {
                    foreach (var errorMessage in result.Errors) ModelState.AddModelError("", errorMessage.Description);
        ...
        public async Task<IActionResult> ConfirmEmail(string uid, string token, string email)
        {
            ...
            if (!string.IsNullOrEmpty(uid) && !string.IsNullOrEmpty(token))
            {
                ...
                if (result.Succeeded) model.EmailVerified = true;
            }
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordModel model)
        {
            if (ModelState.IsValid)
            {
                // code here
                var user = await _loginRepository.GetUserByEmailAsync(model.Email);
                if (user != null) await _loginRepository.GenerateForgotPasswordTokenAsync(user);
