    ...
    public class HomeController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            for (var i = 1; i <= 12; i++)
            {
                ...
                var od = _context.Orders.Where(o => o.Created_date.Month == i && o.Created_date.Year == now.Year)
                    .ToList();
                float revenue = 0;
            ...
            // Tính giá vốn
            var stocks = _context.StockReceived.Where(s => s.Date.Year == now.Year && s.Date.Month == now.Month)
                .ToList();
            report.CostPrice = 0;
            ...
            foreach (var product in products)
            {
                ...
                foreach (var item in productSizes) totalStock += item.Quantity;
            }

            // Tính số lượng nhập kho
            var received = _context.StockReceivedDetail.ToList();
            var totalReceived = 0;
            foreach (var item in received) totalReceived += item.Quantity;
            ...
            {
                var stock = (int)(float)(totalStock * 100 / totalReceived);
                ViewBag.Stock = stock;
            }
            ...
            if (star.Count != 0)
            {
                ...
                foreach (var item in star)
                    if (item.Rate == 1)
                        ...
                        fiveStar++;
                ViewBag.OneStar = (int)(oneStar * 100 / star.Count);
                ViewBag.TwoStar = (int)(twoStar * 100 / star.Count);
                ViewBag.ThreeStar = (int)(threeStar * 100 / star.Count);
                ViewBag.FourStar = (int)(fourStar * 100 / star.Count);
                ViewBag.FiveStar = (int)(fiveStar * 100 / star.Count);
            }
            else
            {
            ...
            if (_context.Promotion.Where(p => p.StartedDate <= DateTime.Now && p.EndDate >= DateTime.Now).ToList()
                    .Count != 0)
            {
                var promotion = _context.Promotion
                    .Where(p => p.StartedDate <= DateTime.Now && p.EndDate >= DateTime.Now).ToList();
