...
namespace TracyShop.Areas.Admin.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        public IActionResult ReportByMonth()
        {
            ...
            for (var i = 1; i <= 12; i++) report.Months.Add(i);
            report.Years = new List<int>();
            for (var i = 1970; i <= now.Year; i++) report.Years.Add(i);
        ...
        public IActionResult ReportByMonth(ReportViewModel reportView)
        {
            ...
            for (var i = 1; i <= 12; i++) report.Months.Add(i);
            report.Years = new List<int>();
            for (var i = 1970; i <= DateTime.Now.Year; i++) report.Years.Add(i);
        ...
        private float RevenueByMonth(int month, int year)
        {
            var orders = month > 0
                ? _context.Orders.Where(o =>
                        o.Created_date.Month == month && o.Created_date.Year == year && o.Status < 4 && o.Status > 0)
                    .ToList()
        ...
        {
            var stocks = month > 0
                ? _context.StockReceived.Where(s => s.Date.Month == month && s.Date.Year == year).ToList()
