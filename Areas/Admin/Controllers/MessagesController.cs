...
namespace TracyShop.Areas.Admin.Controllers
{
    ...
    public class MessagesController : Controller
    {
        ...
        public async Task<IActionResult> Chat(string userId)
        {
            ...
            ViewBag.Message = "";
            ViewBag.Start = _context.Messages.FirstOrDefault(m => m.RequestMessage.ToLower().Contains("Bắt đầu"))
                .ResponseMessage;
            ...
            if (userId != null)
            {
                ...
                if (messages.Count > 0) await SeenMessages(messages);
            ...
            else
            {
                ...
                // cập nhật lại trạng thái của tin nhắn thành đã xem
                var messages = _context.Chats.Where(m => m.IsSeen == false && m.UserId.Contains(lastUserId))
                    .ToList();
                if (messages.Count > 0) await SeenMessages(messages);
        ...
        {
            var chat = _context.Chats.OrderBy(c => c.CreatedAt)
                .LastOrDefault(c => c.UserId.Contains(responseMessage.UserId));

            if (chat == default) return NotFound();
        ...
        {
            var userChats = new List<UserChatViewModel>();
            if (_context.Chats.ToList().Count != 0)
            {
                var users = _context.Chats.OrderByDescending(c => c.CreatedAt).Select(c => c.UserId).Distinct()
                    .ToList();
                ...
                foreach (var item in users)
                {
                    ...
                    userChats.Add(new UserChatViewModel
                    {
                        ...
                        LastMessage = _context.Chats.OrderByDescending(c => c.CreatedAt)
                            .FirstOrDefault(c => c.UserId.Contains(user.Id)).Request
                    });
                }
            }
